		...
		Registry.load(
				...
				parameters).then(
						(_) =>
								Registry.openScope(
										Scope.ISOLATE)).then(
												(_) => Registry.lookupObject(DirectHandler).directCall(new DevServerDirectCall(message))).catchError((error) {
...
class DevDirectServer extends AbstractDirectServer {
	...
	void handleRequest(String base, String application, String path, HttpRequest request) {
...
class DirectServer extends AbstractDirectServer {
	...
	DirectServer({String host: "0.0.0.0", num port: 8081, Uri webUri, this.module, this.parameters: const {}}) : super(
			host: host,
			port: port,
			webUri: webUri) {
		DIRECT_ENVIROMENT = DirectEnviroment.SERVER;
	...
	void handleRequest(String base, String application, String path, HttpRequest request) {
		...
		Registry.lookupObject(DirectHandler).directCall(new ServerDirectCall(base, application, path, request));
	}
}
		...
		return HttpServer.bind(_host, _port).then((server) {
			...
			server.listen((HttpRequest request) {
				...
				if (request.method == "OPTIONS") {
					...
					if ((request.uri.pathSegments.length > 0 && request.uri.pathSegments[0] == "direct") ||
							(request.uri.pathSegments.length > 1 && request.uri.pathSegments[1] == "direct")) {
						String application = request.uri.pathSegments.length > 1 && request.uri.pathSegments[1] == "direct" ?
...
class ServerDirectCall implements DirectCall {
	...
	Future onRequest(Future directCall(String base, String application, String path, String json, Map<String,
		...
		if (multipartAction != null) {
			...
			request2.stream.transform(converter).listen((MultipartRequest multipartRequest) {
				buffer.write("""
					{
			...
			request2.stream.transform(UTF8.decoder).listen((String chunk) => buffer.write(chunk));
		}
		...
		request.listen((data) => request2.add(data), onDone: () {
			request2.close().then((_) {
				if (resultFuture != null) {
					...
					return directCall(base, application, path, buffer.toString(), headers, null, (jsonResponse, responseHeaders) {
						responseHeaders.forEach((name, value) => request.response.headers.add(name, value));
						request.response.write(jsonResponse);
						request.response.close();
					});
...
class DevServerDirectCall implements DirectCall {
	...
	Future onRequest(Future directCall(String base, String application, String path, String json, Map<String,
		...
		if (multipartAction != null) {
			MultipartConverter converter = new MultipartConverter(headers);
			request.stream.transform(converter).listen((MultipartRequest multipartRequest) {
				buffer.write("""
						{
							"action": "$multipartAction",
							"method": "$multipartMethod",
			...
			request.stream.transform(UTF8.decoder).listen((String chunk) => buffer.write(chunk));
		}
