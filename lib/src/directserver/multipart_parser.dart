...
class MultipartSink extends ChunkedConversionSink<List<int>> {
	...
	String popHeader() {
		var list = new List.from(headerChunkQueue.expand((chunk) => chunk), growable: false);
	...
	void process() {
		if (this.lastChunk != null && this.lastChunk.isNotEmpty) {
			var index = indexTo(this.lastChunk, this.leftEncodedMarker, this.lastChunkIndex);
			if (index != -1) {
				// trovato matching
				if (this.nextEncodedMarker.length == this.leftEncodedMarker.length) {
					...
					if (newIndex > this.lastChunk.length) {
						...
						this.leftEncodedMarker = this.nextEncodedMarker.sublist(this.lastChunk.length - index);
						this.lastChunk = [];
						this.lastChunkIndex = 0;
					} else {
					...
					if (index == 0) {
						...
						if (newIndex > this.lastChunk.length) {
							...
							this.leftEncodedMarker = this.leftEncodedMarker.sublist(this.lastChunk.length - index);
							this.lastChunk = [];
							this.lastChunkIndex = 0;
						} else {
						...
						pushChunk(this.nextEncodedMarker, 0, this.nextEncodedMarker.length - this.leftEncodedMarker.length);
...
class MultipartRequestImpl implements MultipartRequest {
	...
	StreamController<RequestProgress> _progressController = new StreamController(sync: true);
	StreamController<RequestParameter> _parameterController = new StreamController(sync: true);
	...
	void close() {
		...
		.whenComplete(() => _parameterController.close())
		.whenComplete(() {
			Future.forEach(_allFutures, (future) => future)
			.then((_) {
				_completer.complete();
			}).catchError((error) {
...
class RequestProgressImpl implements RequestProgress {
	...
	RequestProgress update(num value) => new RequestProgressImpl._update(value, this);
...
class RequestParameterImpl implements RequestParameter {
	...
	StreamController<List<int>> _chunkController = new StreamController(sync: true);
	...
	RequestParameterImpl(String header) {
		...
		_LINE_SPLITTER.convert(header).forEach((line) {
			...
			left.split(";").forEach((part) {
				...
				if (i2 != -1) {
					values[part.substring(0, i2)] = part.substring(i2 + 2, part.length - 1);
				} else {
					values[part] = "";
				}
			});
